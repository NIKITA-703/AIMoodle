...
def solve_active_test(driver, lecture_text):
    ...
    while True:
        try:
            time.sleep(2)  # Даем время на прогрузку анимаций

            # --- 1. ПРОВЕРКА НА КОНЕЦ ТЕСТА ---
            # Если нет вопросов, но есть таблица результатов -> сдаем
            ...
            # А) Проверяем Выпадающий список (Select)
            select_elements = q_block.find_elements(By.TAG_NAME, "select")

            # Б) Проверяем Ввод текста (Text Input)
            # Ищем input type='text', у которого имя начинается на 'q' (стандарт Moodle для ответов)
            text_inputs = q_block.find_elements(By.CSS_SELECTOR, "input[type='text'][name^='q']")

            if select_elements:
                question_type = "select"
                select_obj = Select(select_elements[0])
                # Собираем текст опций для ИИ
                for opt in select_obj.options:
                    txt = opt.text.strip()
                    if "выбрать" not in txt.lower() and txt:  # Игнорируем "Выберите..."
                        options_text.append(txt)
                        options_map[txt.lower()] = txt  # Ключ - текст в нижнем регистре

            elif text_inputs:
                question_type = "text"
                # Сохраняем элемент, чтобы потом в него писать
                options_map["text_input"] = text_inputs[0]
                # Опций нет, ИИ должен сгенерировать слово сам

            else:
                # В) Иначе это Радио или Чекбоксы (Старая логика)
            ...
            if question_type == 'text':
                # Очищаем ответ ИИ от кавычек и точек
                text_answer_clean = ai_ans.strip().strip('"').strip("'").strip(".")
                # Убираем префиксы вида "a. ", "a) ", "1. " в текстовом ответе
                ...
                if question_type == 'radio':
                    ...
                    # Чекбоксы: берем ВСЕ уникальные валидные ключи
            ...
            if question_type == 'text':
                ...
                try:
                    # Для селекта target_keys хранит ТЕКСТ опции
            ...
            try:
                ...
                pass  # Возможно, это последний вопрос, и переход произойдет автоматически
